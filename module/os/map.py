...
class OSMap(OSFleet, Map, GlobeCamera, StorageHandler, StrategicSearchHandler):
    ...
    def clear_question_any_fleet(self, drop=None):
        ...
        每清完一个问号会补扫一次当前视野，把清问号后才显现的事件捞出来；
        一旦命中了目标事件（明石/记录塔/信息探测装置）即停止；若只是清掉普通
        问号，则继续切换下一支舰队，直到找到目标或扫完所有舰队。
        ...
        for fleet in fleets:
            ...
            # 清完问号后补扫一次当前视野：把被舰队遮挡、清完问号后才显现的
            # 装置/塔/明石重新捞出来，避免“只清问号就切队”而漏检。
            try:
                ...
                logger.debug(
                    f"[大世界-搜索] 清问号后当前视野重扫异常，继续: {e}", exc_info=True
                )
    ...
    def _execute_fixed_patrol_scan(
        ...
        按“强制移动等级”执行：
        等级 0：关闭，不做任何强制移动；
        等级 1：仅换队重扫 —— 遍历 1~4 舰队雷达找“?”，每清完一个问号补扫一次当前
                 视野；命中目标事件（明石/记录塔/信息探测装置）即处理并止；扫完所有
                 舰队仍未命中则止（不做强制移动）；
